...
# cd /share/project/baishuanghao/code/VLA-TMEE && ./scripts/imbalance/libero/train/train_libero_mee.sh oft libero_spatial
...
imbalance_ratio=0.1
max_train_steps=$(awk "BEGIN { printf \"%d\", $original_train_steps * ($imbalance_ratio + 1) * 0.4 }")
...
for libero_suite in "${suites[@]}"; do
  ...
  /share/project/baishuanghao/miniconda3/envs/vla_tmee/bin/accelerate launch \
    ...
    --datasets.vla_data.use_imbalance true \
    --datasets.vla_data.imbalance_ratio ${imbalance_ratio} \
    ...
    --run_root_dir ./outputs_libero_imbalance/${libero_suite}_${para_type}_${mee_type}_${mee_weight} \
    --run_id ${run_id} \
