...
# cd /share/project/baishuanghao/code/starVLA && ./scripts/action_noise/libero/eval/eval_libero_gr00t.sh

source /share/project/baishuanghao/miniconda3/etc/profile.d/conda.sh
conda activate vla_tmee

export CUDA_VISIBLE_DEVICES=0
ckpt_root=/share/project/baishuanghao/code/starVLA/pretrained_models_noise_action/libero_goal_2B_mee1e-1_tmee
ckpt_path=starvla_qwen_gr00t/final_model/pytorch_model.pt
run_id=$(echo "$ckpt_path" | cut -d'/' -f1)
or_type=impulse    # cauchy impulse 
your_ckpt="$ckpt_root/$or_type/$ckpt_path"
log_path="$ckpt_root/$or_type/$run_id"
