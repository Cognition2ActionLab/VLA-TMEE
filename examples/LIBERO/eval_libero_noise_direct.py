...
from examples.LIBERO.add_noise import *
...
class Args:
    ...
    OR_type: str = "degradation_noise_sAp_1"


def eval_libero(args: Args) -> None:
    OR_type = args.OR_type
    if OR_type != None:
        parts = OR_type.split('_')
    ...
    log_file_name = f"{args.log_path}/libero_eval_{args.task_suite_name}_{run_id}.log"
    if os.path.exists(log_file_name):
        print(f"Log file {log_file_name} already exists. Skipping...")
        return
    ...
    for task_id in tqdm.tqdm(range(num_tasks_in_suite)):
        ...
        for episode_idx in tqdm.tqdm(range(args.num_trials_per_task)):
            ...
            while t < max_steps + args.num_steps_wait:
                ...
                frame_idx = t - args.num_steps_wait + 1
                if OR_type != None:
                    idx_episode = str(args.task_suite_name)+'_'+str(task_id)+'_'+str(episode_idx)
                    img = apply_image_perturbation(
                            image = img,
                            category = OR_type_category,
                            ptype = OR_type_ptype,
                            variant = OR_type_variant,
                            seed = string_to_seed(f"{OR_type_category}_{OR_type_ptype}_{OR_type_variant}_{idx_episode}_{frame_idx}"),
                            frame_idx=frame_idx,
                            interval=OR_type_freq,
                            past_image_list=replay_images if len(replay_images)!=0 else [img]
                        )
                    wrist_img = apply_image_perturbation(
                            image = wrist_img,
                            category = OR_type_category,
                            ptype = OR_type_ptype,
                            variant = OR_type_variant,
