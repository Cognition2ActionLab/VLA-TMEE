...
from vla_tmee.model.modules.action_model.flow_matching_head.action_encoder import (
    SinusoidalPositionalEncoding,
    swish,
)
from vla_tmee.model.modules.action_model.flow_matching_head.cross_attention_dit import DiT
...
class FlowmatchingActionHeadConfig(PretrainedConfig):
    ...
    enable_mee: bool = field(default=False, metadata={"help": "."})
    mee_weight: float = field(default=1e-2, metadata={"help": "."})
...
class FlowmatchingActionHead(nn.Module):
    def __init__(
        ...
        self.mee_type = 'tmee'
        if hasattr(full_config, "enable_mee"):
            self.enable_mee = full_config.enable_mee
        if hasattr(full_config, "mee_type"):
            self.mee_type = full_config.mee_type
    ...
    def forward(self, vl_embs: torch.Tensor, actions: torch.Tensor, state: torch.Tensor = None):
        ...
        noise = torch.randn(actions.shape, device=actions.device, dtype=actions.dtype)  # [B, num_action, 7]
        ...
        model_output = self.model(  # [B, curr_action + future action, feat_dim]
            hidden_states=sa_embs,
            encoder_hidden_states=vl_embs,
            timestep=t_discretized,
            return_all_hidden_states=False,  # NOTE (YL): not using flare now
        )
        pred = self.action_decoder(model_output)        # [B, future action + curr_action , 7]
        ...
        if self.enable_mee:
            if self.mee_type == 'tmee':
                mee_loss = simple_mee_loss(pred_actions, velocity)
            elif self.mee_type == 'cw-tmee':
                mee_loss = adaptive_mee_loss_chunk(pred_actions, velocity)
            elif self.mee_type == 'ew-tmee':
                mee_loss = adaptive_mee_loss_element(pred_actions, velocity)
            else:
