...
import numpy as np
from vla_tmee.utils.img_noise import apply_image_perturbation
...
def get_frames_by_timestamps(
    ...
    video_backend: str = "decord",
    video_backend_kwargs: dict = {},
    enable_image_noise: bool = False,
    image_noise_type: str = None,
) -> np.ndarray:
    ...
    if video_backend == "decord":
        ...
        for frame in reader:
            ...
            if enable_image_noise:
                if image_noise_type != None and np.random.random() < 0.5:
                    parts = image_noise_type.split('_')
        ...
        # frames = [f.cpu().numpy() for f in loaded_frames]  # list of np arrays
