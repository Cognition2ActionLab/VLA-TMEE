...
from vla_tmee.dataloader.gr00t_lerobot.video import get_all_frames, get_frames_by_timestamps

from vla_tmee.dataloader.gr00t_lerobot.embodiment_tags import EmbodimentTag
from vla_tmee.dataloader.gr00t_lerobot.schema import (
    DatasetMetadata,
    DatasetStatisticalValues,
    LeRobotModalityMetadata,
    LeRobotStateActionMetadata,
)
from vla_tmee.dataloader.gr00t_lerobot.transform import ComposedModalityTransform
...
class LeRobotSingleDataset(Dataset):
    ...
    def __init__(
        ...
        self._trajectory_ids, self._trajectory_lengths, self.task_counter = self._get_trajectories()
        self._modality_keys = self._get_modality_keys()
        self._delta_indices = self._get_delta_indices()

        flags = [use_all, use_few, use_imbalance]
        if sum(flags) >= 2:
            use_all = True
            use_few = False
            use_imbalance = False
        print(f"[Data Setting] use_all={use_all}, use_few={use_few}, use_imbalance={use_imbalance}")
        self._all_steps = self._get_all_steps(use_all, use_few, few_shot_ratio, use_imbalance, imbalance_ratio)
    ...
    def _get_trajectories(self) -> tuple[np.ndarray, np.ndarray]:
        ...
        for episode in episode_metadata:
            ...
            task_counter[episode["tasks"][0]] += 1
        return np.array(trajectory_ids), np.array(trajectory_lengths), task_counter

    def _get_all_steps(self, use_all=True, use_few=False, few_shot_ratio=0.2, use_imbalance=False, imbalance_ratio=0.25) -> list[tuple[int, int]]:
        ...
        if use_all:
            ...
            config_key = f"few_{few_shot_ratio}"
        elif use_imbalance:
            config_key = f"imbalance_{imbalance_ratio}"
        ...
        all_steps = self._get_all_steps_single_process(use_all, use_few, few_shot_ratio, use_imbalance, imbalance_ratio)
    ...
    def _get_all_steps_single_process(self, use_all=True, use_few=False, few_shot_ratio=0.2, use_imbalance=False, imbalance_ratio=0.25) -> list[tuple[int, int]]:
        """Original single-process implementation as fallback."""
        all_steps: list[tuple[int, int]] = []
        skipped_trajectories = 0
        processed_trajectories = 0

        current_retained_counts = defaultdict(int)
        if use_few:
            max_retained_counts = {
                task: max(1, int(count * few_shot_ratio)) # 至少保留 1 个，确保 few-shot
                for task, count in self.task_counter.items()
            }
        if use_imbalance:
            most_common_tasks = self.task_counter.most_common(len(self.task_counter)//2)
            least_common_tasks = self.task_counter.most_common()[-len(self.task_counter)//2:]
            max_retained_counts = {}
            for task, _ in most_common_tasks:
                max_retained_counts[task] = min(40, self.task_counter[task])
            for task, _ in least_common_tasks:
                max_retained_counts[task] = min(int(40 * imbalance_ratio), self.task_counter[task])
        ...
        for trajectory_id, trajectory_length in tqdm(zip(self.trajectory_ids, self.trajectory_lengths), total=len(self.trajectory_ids), desc="Getting All Step"):
            ...
            if has_language_modality:
                ...
                try:
                    ...
                    if not language_instruction or language_instruction[0] == "":
                        ...
                        continue
                    # few-shot
                    if use_few and current_retained_counts[language_instruction[0]] >= max_retained_counts[language_instruction[0]]:
                        print(f"Skipping trajectory {trajectory_id} due to few-shot setting")
                        skipped_trajectories += 1
                        trajectory_skipped = True
                        continue
                    # imbalance
                    elif use_imbalance and current_retained_counts[language_instruction[0]] >= max_retained_counts[language_instruction[0]]:
                        print(f"Skipping trajectory {trajectory_id} due to imbalance setting")
                        skipped_trajectories += 1
                        trajectory_skipped = True
                        continue
                except Exception as e:
            ...
            if not trajectory_skipped:
                ...
                current_retained_counts[language_instruction[0]] += 1
        ...
        print(f"Single-process summary: Processed {processed_trajectories} trajectories, skipped {skipped_trajectories} empty or abundant trajectories")
    ...
    def get_step_data(self, trajectory_id: int, base_index: int, enable_image_noise, image_noise_type) -> dict:
        ...
        for modality in self.modality_keys:
            ...
            for key in self.modality_keys[modality]:
                data[key] = self.get_data_by_modality(trajectory_id, modality, key, base_index, enable_image_noise, image_noise_type)
        return data
    ...
    def get_video(
        ...
        return get_frames_by_timestamps(
            ...
            enable_image_noise=enable_image_noise, 
    ...
    def get_data_by_modality(
        ...
        """
        if modality == "video":
            return self.get_video(trajectory_id, key, base_index, enable_image_noise, image_noise_type)
...
class LeRobotMixtureDataset(Dataset):
    ...
    def __init__(
        ...
        self.enable_image_noise = enable_image_noise
    ...
    def __getitem__(self, index: int) -> dict:
        ...
        for attempt in range(max_retries):
            try:
                ...
                data = dataset.transforms(dataset.get_step_data(trajectory_name, step, self.enable_image_noise, self.image_noise_type))
